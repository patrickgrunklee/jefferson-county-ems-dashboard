"""
Regenerate the Regional Overnight ALS Hub Coverage Map with improved detail.
- County boundary overlay from jefferson_county.geojson
- Better label placement, avoiding overlaps
- Hub zones shaded with convex hulls
- Richer annotations (call volume, service level, drive time on lines)
- Cleaner styling
"""
...
# ── Data (from staffing_reallocation_analysis.py) ────────────────────────
...
# Hub assignments (from the analysis)
...
# Load distance matrix for assignments
...
county_geojson_path = os.path.join(SCRIPT_DIR, "jefferson_county.geojson")
...
HUB_COLORS = {
    "Watertown": "#c0392b",     # deep red
    "Fort Atkinson": "#2471a3", # deep blue
    "Edgerton": "#1e8449",      # deep green
}
HUB_LIGHT = {
    "Watertown": "#f5b7b1",
...
# Draw hub coverage zones (convex hulls of hub + assigned depts)
for hub in HUB_CANDIDATES:
...
# Draw connection lines (hub → dept) with drive time labels
for dept, info in assignments.items():
    hub = info["hub"]
    lat, lon = DEPT_COORDS[dept]
    hub_lat, hub_lon = DEPT_COORDS[hub]
...
# Label placement offsets to avoid overlaps (manually tuned)
LABEL_OFFSETS = {
    "Watertown":     (18, 10),
    "Fort Atkinson": (18, -16),
    "Edgerton":      (-22, -18),
...
for dept, (lat, lon) in DEPT_COORDS.items():
    ...
    if dept in HUB_CANDIDATES:
        ...
        # Covered department: colored circle matching its hub
        hub = assignments[dept]["hub"]
        color = HUB_COLORS[hub]
        # Scale marker by call volume
        ...
        # Other department (not in hub system): gray triangle
...
legend_items = [
    ...
    plt.Line2D([0], [0], marker="s", color="w", markerfacecolor=HUB_COLORS["Fort Atkinson"],
               markersize=14, markeredgecolor="black", markeredgewidth=1.5,
               label=f"Fort Atkinson — Central Hub (16 FT, 3 amb)"),
    plt.Line2D([0], [0], marker="s", color="w", markerfacecolor=HUB_COLORS["Edgerton"],
               markersize=14, markeredgecolor="black", markeredgewidth=1.5,
               label=f"Edgerton — South Hub (24 FT, 2 amb)"),
...
summary = (f"Coverage Summary\n"
           f"3 hubs serve {n_covered} departments\n"
...
plt.tight_layout()
out_path = os.path.join(SCRIPT_DIR, "reallocation_hub_coverage_map_v2.png")
